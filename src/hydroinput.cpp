...
void findInterpMeteo(globalvar& gv,double *tcum)
{
    ...
    for(int a=0;a<nprec;a++){
        meteo_t_i = (*gv.meteoall_int).at(a,0);
        meteo_t_i2 = (*gv.meteoall_int).at(a+1,0);
        
        if ((*tcum) > meteo_t_i && (*tcum) <= meteo_t_i2) {
            if ((meteo_t_i2 - meteo_t_i) > 0){
                temp_calc_i = (*gv.meteoall_int).at(a,1);
                rainfall_calc_i = (*gv.meteoall_int).at(a,2);
                snowfall_calc_i = (*gv.meteoall_int).at(a,3);
                precipt_c_calc_i = (*gv.meteoall_int).at(a,4);
...
void findInterpQmelt(globalvar& gv,double *tcum)
{
    ...
    for(int a=0;a<nqmlt;a++){
        qmlt_t_i = (*gv.qcmel_int).at(a,0);
        qmlt_t_i2 = (*gv.qcmel_int).at(a+1,0);
        qmlt_i = (*gv.qcmel_int).at(a,1);
