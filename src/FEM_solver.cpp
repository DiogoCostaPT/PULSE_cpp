...
arma::mat c_new = (*gv.c_m)(arma::span(0,nli-1),arma::span(0,(gv.wetfront_cell)-1));
...
for (ih=1;ih<nhi-1;ih++){
    for (il=1;il<nli-1;il++){
        ...
        if ((*gv.velc_2d)(il,ih-1) > 0){
            ...
            dcdy(il,ih)=(c_new(il,ih)-c_new(il,ih-1))/gv.snowh;
            dc2dy2(il,ih)=((c_new(il,ih+1)-c_new(il,ih))/gv.snowh-(c_new(il,ih)-c_new(il,ih-1))/gv.snowh)/(gv.snowh);         
            dc2dx2(il,ih)=((c_new(il+1,ih)-c_new(il,ih))/gv.snowl-(c_new(il,ih)-c_new(il-1,ih))/gv.snowl)/(gv.snowl);
            dcdt(il,ih)=-(*gv.velc_2d)(il,ih)*dcdy(il,ih)+(*gv.disp_2d)(il,ih)*(dc2dx2(il,ih)+dc2dy2(il,ih));

            c_new(il,ih)=c_new(il,ih)+dcdt(il,ih)*(*deltt);
...
c_new(0,arma::span(0,nhi-1))=c_new(1,arma::span(0,nhi-1));  
c_new(nli-1,arma::span(0,nhi-1))=c_new(nli-2,arma::span(0,nhi-1));   
    
(*gv.c_m)(arma::span(0,nli-1),arma::span(0,gv.wetfront_cell-1)) =  c_new;
