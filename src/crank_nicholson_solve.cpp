...
void crank_nicholson(globalvar& gv,double *deltt)
{
    ...
    int nt = nli*nhi;
    arma::mat k1 = (*gv.velc)(arma::span(0,nli-1),arma::span(0,nhi-1))*(*deltt)/(4*gv.snowh);       // constants for Crank-Nicholson scheme
    arma::mat k2 = (*gv.disp)(arma::span(0,nli-1),arma::span(0,nhi-1))*(*deltt)/(2*pow(gv.snowh,2));     // constants for Crank-Nicholson scheme
    arma::mat k3 = (*gv.disp)(arma::span(0,nli-1),arma::span(0,nhi-1))*(*deltt)/(2*pow(gv.snowl,2));     // constants for Crank-Nicholson scheme
    ...
    A = arma::diagmat(a3_r(0,arma::span(0,nt-nli-1))%arma::ones(1,nt-nli),nli)
            +arma::diagmat(a1_r(0,arma::span(0,nt-nli-1))%arma::ones(1,nt-nli),-nli)
            +arma::diagmat(a2_r(0,arma::span(0,nt-1))%arma::ones(1,nt))
            -arma::diagmat(k3_r(0,arma::span(0,nt-2))%arma::ones(1,nt-1),1);
            -arma::diagmat(k3_r(0,arma::span(0,nt-2))%arma::ones(1,nt-1),-1);
    ...
    B = -arma::diagmat(a3_r(0,arma::span(0,nt-nli-1))%arma::ones(1,nt-nli),nli)
            -arma::diagmat(a1_r(0,arma::span(0,nt-nli-1))%arma::ones(1,nt-nli),-nli)
            +arma::diagmat(a4_r(0,arma::span(0,nt-1))%arma::ones(1,nt))
            +arma::diagmat(k3_r(0,arma::span(0,nt-2))%arma::ones(1,nt-1),1);
            +arma::diagmat(k3_r(0,arma::span(0,nt-2))%arma::ones(1,nt-1),-1);
